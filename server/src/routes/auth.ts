...
router.post("/register", async (req: AuthRequest, res: Response) => {
  try {
    ...
    const token = await import("../auth.js").then(m => m.generateToken({
...
router.post("/wechat", async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!WECHAT_APP_ID || !WECHAT_APP_SECRET) {
      res.status(500).json({ error: "微信登录未配置，请联系管理员" });
      return;
    }
    ...
    if ((tokenData as any).errcode) {
      res.status(400).json({ error: (tokenData as any).errmsg || "微信授权失败" });
      return;
    }
    ...
    if ((userData as any).errcode) {
      res.status(400).json({ error: (userData as any).errmsg || "获取微信用户信息失败" });
      return;
    }
