...
router.post("/", requireAuth, async (req: AuthRequest, res: Response) => {
  ...
  await query(
    "INSERT INTO outfits (id, user_id, name, occasion, weather, is_public) VALUES (?, ?, ?, ?, ?, ?)",
    [id, req.userId!, name || null, occasion || null, weather || null, is_public ? 1 : 0],
  );
  ...
  if (items && Array.isArray(items)) {
    for (let idx = 0; idx < items.length; idx++) {
      ...
      await query(
        "INSERT INTO outfit_items (id, outfit_id, clothing_item_id, reason, position) VALUES (?, ?, ?, ?, ?)",
        [uuid(), id, item.clothing_item_id, item.reason || null, idx],
      );
    }
  }
...
router.post("/:id/like", requireAuth, async (req: AuthRequest, res: Response) => {
  await query("UPDATE outfits SET likes_count = likes_count + 1 WHERE id = ?", [req.params.id]);
