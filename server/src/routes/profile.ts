...
router.put("/", requireAuth, async (req: AuthRequest, res: Response) => {
  ...
  await query(
    ...
    [username || null, avatar_url || null, req.userId!],
  );

  await query(
    "UPDATE profiles SET bio = COALESCE(?, bio), height = COALESCE(?, height), weight = COALESCE(?, weight), body_type = COALESCE(?, body_type), style_tags = COALESCE(?, style_tags), is_public = COALESCE(?, is_public), updated_at = datetime('now') WHERE id = ?",
    [bio || null, height || null, weight || null, body_type || null, style_tags ? JSON.stringify(style_tags) : null, is_public !== undefined ? (is_public ? 1 : 0) : null, req.userId!],
  );
